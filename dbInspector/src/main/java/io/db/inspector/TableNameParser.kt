...
class TableNameParser(sql: String) {
    ...
    init {
        ...
        val tokens = cleansed.split(REGEX_SPACE.toRegex()).dropLastWhile { it.isEmpty() }
            .toTypedArray()
        var index = 0
    ...
    private fun shouldProcessMultipleTables(nextToken: String?): Boolean = nextToken != null && nextToken == TOKEN_COMMA
    ...
    companion object {
        ...
        private val concerned =
            Arrays.asList(KEYWORD_TABLE, KEYWORD_INTO, KEYWORD_JOIN, KEYWORD_USING, KEYWORD_UPDATE)
        private val ignored = Arrays.asList(TOKEN_PARAN_START, TOKEN_SET, TOKEN_OF, TOKEN_DUAL)
